//! In-process mocks for the Chrome DevTools Protocol HTTP and WebSocket
//! surfaces.
//!
//! These mocks are used by `cdp-browser-lite`'s own integration tests and
//! are re-exported through `cdp_browser_lite::test_support` (gated by the
//! `test-support` cargo feature) so downstream crates can drive the library
//! without spawning a real browser.
//!
//! Two pieces are provided:
//!
//! - [`MockChrome`]: a minimal HTTP peer that responds to the
//!   `/json/version` probe and to `Browser.getVersion` over the page
//!   WebSocket. Configurable via [`MockBehavior`].
//! - [`MockDevTools`]: a richer mock that exposes `/json/version`,
//!   `/json/list`, `/json/new`, and a full WebSocket CDP endpoint that
//!   dispatches `Browser.*` and `Target.*` commands. Configurable via
//!   [`MockWsBehavior`].
//!
//! Both mocks bind ephemeral ports, drop the port on `Drop`, and report
//! connection accounting so tests can assert reconnect behaviour.
...
fn dispatch_command(registry: &mut TargetRegistry, method: &str, params: &Value) -> Value {
    match method {
        ...
        "Runtime.evaluate" => {
            if let Some(expr) = params.get("expression").and_then(Value::as_str) {
                if expr == "document.documentElement.outerHTML" {
                    json!({
                        "result": {
                            "type": "string",
                            "value": "<html><body><h1>Hello World</h1><div id='test'>This is a test</div><input id='test-input' value='' /></body></html>"
                        }
                    })
